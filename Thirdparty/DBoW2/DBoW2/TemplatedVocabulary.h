                                                      ...
                                                      const vector<pDescriptor> &descriptors, int current_level)
{
    ...
    if ((int)descriptors.size() <= m_k)
    {
        ...
        for (unsigned int i = 0; i < descriptors.size(); i++)
        {
            ...
            clusters.push_back(*descriptors[i]); // clusters存了描述子，最后是虚拟描述子
        }
    }
    else
    {
        ...
        bool first_time = true; // 维护first_time从而实现第一次时随机设定聚类中心
        bool goon = true;

        // to check if clusters move after iterations
        vector<int> last_association, current_association;

        // 开始迭代，由于是一个树，所以需要分层递归实现
        // 递归终止条件：只有当goon为false时停止迭代
        while (goon)
        {
            ...
            {
                // random sample
                // 如果是第一次，随机设定初始聚类中心，使用Kmeans++方法选择聚类中心
            ...
            else
            {
                ...
                for (unsigned int c = 0; c < clusters.size(); ++c)
                {
                    ...
                    */
                    ...
                    // 首先按照上一次聚类运行结果，保存各个类(group)里的描述子
                    ...
                    F::meanValue(cluster_descriptors, clusters[c]); // 然后计算聚类中心保存到clusters[c]中
                }
            ...
            //unsigned int d = 0;
            // 计算每个描述子与各个聚类中心的距离，选择最小的
            for (fit = descriptors.begin(); fit != descriptors.end(); ++fit) //, ++d)
            {
                double best_dist = F::distance(*(*fit), clusters[0]); // 以第一个聚类中心与描述子的距离作为初值
                ...
                // 注意：最后存下来的不是iterator，而是相对id，这样可以防止出错
                // 下面两个相当于是正向和反向索引，其中第二个用于判断聚类是否不再改变，从而确定迭代停止时机
                groups[icluster].push_back(fit - descriptors.begin()); // 存了每个group里有哪些描述子
                current_association[fit - descriptors.begin()] = icluster; // 存了每个描述子对应哪个group
            }
            ...
            else
            {
                ...
                // 终止条件：goon变为false，两次迭代结果一样，即收敛时
                goon = false;
                for (unsigned int i = 0; i < current_association.size(); i++)
                {
                    // 比较上一次和这一次聚类的结果，如果不同则继续迭代
            ...
            if (goon)
            {
                ...
                last_association = current_association; // 更新迭代结果
    ...
    // create nodes
    // 一层训练完之后，创建词典树，维护叶子与父节点的关系
    // TO-DO: 这部分建树的方法等回顾下数据结构知识后再来比对下
    for (unsigned int i = 0; i < clusters.size(); ++i)
    {
        NodeId id = m_nodes.size();
        m_nodes.push_back(Node(id));
        m_nodes.back().descriptor = clusters[i];
        m_nodes.back().parent = parent_id; // 递归的输入
    ...
    if (current_level < m_L)
    {
        ...
        for (unsigned int i = 0; i < clusters.size(); ++i)
        {
            ...
            {
                HKmeansStep(id, child_features, current_level + 1); // 递归建树
            }
        }
    }
}
...
void TemplatedVocabulary<TDescriptor, F>::createWords()
{
    ...
    if (!m_nodes.empty())
    {
        ...
        {
            if (nit->isLeaf()) // 只保存词典树的叶子节点，作为word
            {
...
void TemplatedVocabulary<TDescriptor, F>::setNodeWeights(const vector<vector<TDescriptor> > &training_features)
{
    ...
    // TO-DO: 设置权重部分需要再深刻理解TF-IDF原理再回来学习
    ...
    else if (m_weighting == IDF || m_weighting == TF_IDF)
    {
        ...
        for (mit = training_features.begin(); mit != training_features.end(); ++mit)
        {
            ...
            {
                WordId word_id;
                // 找到描述子所对应的word，从而计算词频
                transform(*fit, word_id); // Returns the word id associated to a feature
                                                    ...
                                                    WordId &word_id, WordValue &weight, NodeId *nid, int levelsup) const
{
    ...
    // 分层寻找与描述子最接近的word
    // TO-DO: 这里也是一个经典的树遍历问题，回头再来仔细对照学习
    do
    {
        ++current_level;
    ...
    } while (!m_nodes[final_id].isLeaf()); // 终止条件：到达了叶子节点
